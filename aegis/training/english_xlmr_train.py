#!/usr/bin/env python3
"""
English XLM-RoBERTa Training Script
Trains English hate speech detection model using HateXplain dataset with XLM-RoBERTa
"""
...
# Import our English dataset
from aegis.datasets.english_dataset import EnglishHateXplainDataset

# Set environment variables
os.environ["TOKENIZERS_PARALLELISM"] = "false"

class EnglishXLMDetector(nn.Module):
    """English hate speech detector with XLM-RoBERTa"""
    
    def __init__(self, model_name: str = "xlm-roberta-base", num_targets: int = 10):
        super().__init__()
        ...
        # Task-specific heads
        self.intensity_head = nn.Linear(hidden_size, 6)  # normal, offensive, L1_hate, L2_hate, mild, severe
...
class EnglishXLMTrainer:
    """English XLM-RoBERTa Model Trainer"""
    
    def __init__(self, model_name: str = "xlm-roberta-base", device: str = "cuda"):
        self.device = torch.device(device if torch.cuda.is_available() else "cpu")
        self.model_name = model_name
        
        print(f"🇺🇸 Initializing English XLM-RoBERTa trainer")
        print(f"📱 Device: {self.device}")
        print(f"🤖 Model: {model_name}")
        
        # Load datasets
        self.train_dataset = EnglishHateXplainDataset("train", tokenizer_name=model_name)
        self.test_dataset = EnglishHateXplainDataset("test", tokenizer_name=model_name)
        
        # Create model with correct number of target categories
        num_targets = len(self.train_dataset.target_categories)
        self.model = EnglishXLMDetector(model_name=model_name, num_targets=num_targets).to(self.device)
        ...
        # Loss functions with class weights for English data
        self.intensity_loss = nn.CrossEntropyLoss()
        
        # BIO loss with class weights (English has different distribution)
        bio_class_weights = torch.tensor([1.0, 15.0, 10.0, 20.0, 12.0]).to(self.device)  # O, B-SOFT, I-SOFT, B-HARD, I-HARD
    ...
    def simple_collate_fn(self, batch):
        ...
        intensity_labels = torch.stack([item["sentence_labels"] for item in batch])
    ...
    def evaluate(self):
        ...
        # Multi-label target classification metrics
    ...
    def train(self, epochs: int = 5, patience: int = 2):
        ...
        print(f"🏋️ Starting English XLM-RoBERTa training for {epochs} epochs with Early Stopping...")
        ...
        for epoch in range(epochs):
            ...
            if current_f1 > best_f1:
                ...
                best_epoch = epoch + 1
                patience_counter = 0
                self.save_model(f"english_xlmr_model.pt")
        ...
        history = {
            ...
            'model_config': {
                ...
                'class_weights': [1.0, 15.0, 10.0, 20.0, 12.0]
            }
        }
        
        with open('english_xlmr_training_history.json', 'w') as f:
            json.dump(history, f, indent=2)
        
        print(f"\n✅ English XLM-RoBERTa training completed!")
        print(f"🏆 Best BIO F1: {best_f1:.4f} at Epoch {best_epoch}")
        print(f"📊 Total epochs: {len(train_history)}/{epochs}")
        print(f"📁 Model saved: english_xlmr_model.pt")
        print(f"📁 History saved: english_xlmr_training_history.json")
...
def main():
    parser = argparse.ArgumentParser(description="English XLM-RoBERTa training with Early Stopping")
    ...
    print("🇺🇸 Starting English XLM-RoBERTa Training with Early Stopping...")
    print(f"⚙️ Config: Max {args.epochs} epochs, Patience {args.patience}, {args.model}, {args.device}")
    print("=" * 60)
    
    # Initialize trainer
    trainer = EnglishXLMTrainer(model_name=args.model, device=args.device)
    
    # Train model
    history = trainer.train(epochs=args.epochs, patience=args.patience)
    
    print("🎉 English XLM-RoBERTa training completed successfully!")
